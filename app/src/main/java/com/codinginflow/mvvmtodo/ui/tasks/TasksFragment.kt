...
class TasksFragment : Fragment(R.layout.fragment_tasks) ,TaskAdapter.OnItemClickListener{
            ...
            viewModel.taskEvent.collect { event ->
                when (event) {
                    ...
                    is TasksViewModel.TaskEvent.NavigateToAddTaskScreen -> {
                        val action =
                            TasksFragmentDirections.actionTasksFragment2ToAddEditTaskFragment(null,"New Task")
                        findNavController().navigate(action)
                    }
                    is TasksViewModel.TaskEvent.NavigateToEditTaskScreen -> {
                        val action =
                            TasksFragmentDirections.actionTasksFragment2ToAddEditTaskFragment(event.task,"Edit Task")
                        findNavController().navigate(action)
    ...
    return   when (item.itemId) {
        ...
        R.id.actiom_sort_by_date_created -> {
